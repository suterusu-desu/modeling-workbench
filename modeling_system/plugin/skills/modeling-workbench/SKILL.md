...
For edits affecting achieved shape, use [required outcomes and retained methods](references/preservation-and-lessons.md). Bind protected evaluated regions/poses and their guide sources outside ranked history, consume constraints in the actual preparation adapter, and measure preservation against the exact result. Enable the workspace omission guard. Jev can judge applicability and route a scoped missing prerequisite; it cannot waive checks. Import reviewed techniques and failures into durable learning, with knowledge status separate from candidate success and user rejection authoritative.

Use [executable methods](references/method-control.md) to register genuine preparation, diagnosis and recovery alternatives with MethodCatalog. OperatingSession carries relevant actual reviews into later choices automatically; use [retained learning](references/retained-learning.md) for conditional lessons, cross-case evidence and one-time historical import. RetainedContext can add deliberately projected source passages. Use saved-array preparation and effect/active-surface/pose evidence where it changes the next edit; keep fixed continuation in code. Start an intervention timer when Astra must author a missing capability or interpret evidence, then close it with the resulting artifact. Count actual Jev decisions separately from singleton execution.

For repeated modeling mechanisms use [parameterized operations](references/operation-recipes.md): bind meaningful method alternatives as data, reuse guide-landmark/material-trajectory math, compare actual face-owning geometry, and execute NativeJob/RetainCheckpoint through the existing controller. Retention reuses returned fresh states; qualified adapters can opt into fixed synchronous retention with fresh boundary checks, restoration-content checks and durable no-replay phases. Native guards, independent reopen and actual visual review remain required. Begin image review with `session.begin_review(task)` so workload coverage is retained automatically through submission. Compare the overall affected region and motion to the applicable guide and an earlier useful baseline; local gains must not accumulate visible regressions or override user rejection.
