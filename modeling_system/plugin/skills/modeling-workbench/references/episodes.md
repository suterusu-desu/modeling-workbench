...
Use the project directory as the working directory. For this local Windows installation the existing interpreter is `PROJECT/modeling_system/.runtime/Scripts/python.exe`; another machine may use any environment installed from the package requirements. Replace PROJECT with the absolute bound project, not an arbitrary worktree missing private evidence.
